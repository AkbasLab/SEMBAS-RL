...
def train_random(sim: Simulation, num_episodes: int = None, num_steps: int = None):
    ...
    """
    assert num_episodes or num_steps, "Must specify either num eps or steps"
    ...
    ep_est = num_steps // 50 if num_steps else None
    while (num_episodes is None or ep_i < num_episodes) and (
        num_steps is None or total_steps < num_steps
    ):
        # longitude: float, latitude: float, dir_angle_offset: float, speed: float
        # sim.sim_random_reset()
        agent.update_expl_noise(ep_i, num_episodes or ep_est)
        ...
        print(
            f"{ep_i+1} steps {total_steps} : {sum(step_log[-10:]) / len(step_log[-10:])}, {min(step_log[-10:])}, {max(step_log[-10:])}"
        )
...
def run_sembas_episode(
    ...
    step_limit = step_limit or MAX_STEPS
...
def rerun_and_train(
    ...
    for i, x in enumerate(requests):
        # longitude: float, latitude: float, dir_angle_offset: float, speed: float
        # sim.sim_random_reset()
        agent.update_expl_noise(ep + i, 150)
...
def sembas_reacquisition(
    ...
    new_train_data = run_until_phase(
        session, sim, crit_step_c, api.SembasSession.PHASE_BOUNDARY_EXPL, logger
    )
...
def sembas_training(
    ...
    random_spacing: int = None,
):
    assert random_size is None or (random_size and random_spacing)
    assert batch_size or random_spacing
    ...
    batch_size = batch_size or (random_spacing // crit_step_c) * 2
    ...
    try:
        while (
            (num_iterations is None or i < num_iterations)
            and not (
                target_training_steps is not None
                and num_train_steps >= target_training_steps
            )
            or process == "Training"
        ):
            match process:
                case "NewSearch":
                    ...
                    run_until_phase(
                        ...
                        sim,
                        crit_step_c,
                        api.SembasSession.PHASE_BOUNDARY_EXPL,
                        logger,
                    )
                    process = "BE"
                ...
                case "BE":
                    if session.expect_phase() == api.SembasSession.PHASE_BOUNDARY_EXPL:
                        if target_training_steps is not None:
                            step_limit = target_training_steps - num_be_steps
                        else:
                            step_limit = None

                        if random_size:
                            step_limit = min(
                                step_limit or random_spacing, random_spacing
                            )

                        x, cls, train_data = run_sembas_episode(
                            session, sim, crit_step_c, step_limit=step_limit
                        )
                        ...
                        num_be_steps += len(train_data)
                        if (
                            # target_training_steps is not None
                            # and num_train_steps + num_be_steps >= target_training_steps
                            target_training_steps is not None
                            and num_be_steps >= random_size * 2
                            or target_training_steps is None
                            and len(requests) >= batch_size
                        ):
                            process = "Training"
                    else:
                        logger.info(
                ...
                case "Training":
                    ...
                    if target_training_steps is not None:
                        step_limit = target_training_steps - num_train_steps
                    else:
                        step_limit = None

                    if random_size:
                        step_limit = min(step_limit or random_spacing, random_spacing)
                        logger.info("Random")
                        _, _s = train_random(sim, num_steps=random_size)
                        debug_sim_count += sum(_s)

                    logger.info("Boundary")
                    ep_rewards, ep_steps = rerun_and_train(
                        sim, requests, ep, limit_steps=step_limit
                    )
                    num_train_steps += sum(ep_steps)
                    debug_sim_count += sum(ep_steps)
...
def review_last(num_runs: int):
    sr = 0
    rr = 0
    for i in range(num_runs):
        fn = f"run-{i}.json"
        with open(f".results/sembas-test/{fn}") as f:
            sr += json.load(f)["test-mean"]
        with open(f".results/trdtest/{fn}") as f:
            rr += json.load(f)["test-mean"]

    print(f"Sembas: {sr / num_runs}")
    print(f"Random: {rr / num_runs}")


if __name__ == "__main__":
    review_last(15)
    # wup_path = ".models/warmup/test/agent_warmup-0.pt"
    ...
    #     target_training_steps=1000,
    #     random_size=150,
    #     random_spacing=150,
    # )

    # sim.agent.load("checkpoints/agent_latest.pt")
    # print(perf_test(sim, 20))
    # watch(sim)
